...
async function appendLead({ name, email, tool, summary, answers, timestamp, blobUrl, utmSource, utmCampaign, businessName, revenueBand, revenueRange, ownership, tierInterest, businessConstraint, timeline, reason }) {
  ...
  if (tool === "constraint-roadmap") {
    ...
    const cohortLastColumn = Math.max(ownershipColumn.index, tierInterestColumn.index);
  ...
  if (tool === "cohort-waitlist") {
    ...
    const aggregatedLastColumn = Math.max(ownershipColumn.index, tierInterestColumn.index);
