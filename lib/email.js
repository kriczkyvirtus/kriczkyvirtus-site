...
const TOOL_DETAILS = {
  ...
  "reinvest-harvest": {
    ...
    previewText: "Where the next dollar should go — and why.",
    heading: "Your Reinvest or Harvest Scorecard",
    ...
    buttonText: "View Your Results",
    ctaUrl: "https://www.kriczkyvirtus.com/free-session",
    ctaText: "P.S. — ready to decide where the next dollar should go with your real numbers?",
    ctaButtonText: "Book your free working session",
  },
  "recurring-revenue-12cs": {
    subject: "Your 12 Cs Recurring Revenue Score",
    previewText: "Your scored 15-page recurring revenue roadmap",
    heading: "Your Recurring Revenue Score Is Ready",
    description: "Here's your scored copy of the 12 Cs roadmap — your twelve dimension scores, your total stickiness score, and the specific action steps under each one.",
    buttonText: "View Your Scored Roadmap",
    ctaUrl: "https://www.kriczkyvirtus.com/free-session",
    ctaText: "P.S. — want help turning the two lowest scores into a plan?",
    ctaButtonText: "Book Your Free Working Session",
    bcc: "ekriczky@kriczkyvirtus.com",
  },
};

const COHORT_CONFIRMATION_TEXT = ({ firstName }) => `Hi ${firstName},
...
function buildBrandedEmailHtml({
  ...
  ctaUrl,
  ctaButtonText,
  signoffIntro = "Hopefully this helps you build financial freedom through your business,",
  includeCta = true,
  includeUnsubscribe = true,
}) {
  const renderedDescription = descriptionHtml || `              <p style="font-size: 15px; color: #8B95A5; margin: 0 0 32px 0; line-height: 1.6;">
                ${description}
              </p>`;
  const primaryButtonMarkup = includeCta ? `              <!-- Primary Button -->
  ...
  const ctaMarkup = includeCta ? `              <!-- Divider -->
              ...
              <table cellpadding="0" cellspacing="0">
                <tr>
                  <td style="border: 1px solid rgba(34,211,238,0.3); border-radius: 10px; padding: 12px 24px; background: rgba(34,211,238,0.06);">
                    <a href="${ctaUrl}" target="_blank" style="color: #22D3EE; font-size: 13px; font-weight: 600; text-decoration: none;">
                      ${ctaButtonText}
                    </a>
                  </td>
                </tr>
              </table>` : "";
  const unsubscribeMarkup = includeUnsubscribe ? `              <!-- Unsubscribe -->
              ...
              <table cellpadding="0" cellspacing="0" width="100%" style="margin: 0 0 32px 0;">
                <tr>
                  <td style="padding-bottom: 2px;">
                    ...
                    <p style="font-size: 14px; color: #E8ECF1; margin: 0 0 12px 0; font-weight: 600;">
                ...
                <tr>
                  <td align="center">
                    <img src="https://www.kriczkyvirtus.com/images/edward-kriczky-headshot.jpg" alt="Edward Kriczky" width="64" height="64" draggable="false" style="width: 64px; height: 64px; border-radius: 50%; display: block; object-fit: cover; pointer-events: none;" />
                  </td>
                </tr>
              </table>
...
async function sendResultsEmail({ name, email, tool, resultsUrl }) {
  // Check if email is on the unsubscribe list before sending
  try {
    ...
    const unsubscribedEmails = (result.data.values || []).flat().map(e => e.toLowerCase());
    if (unsubscribedEmails.includes(email.toLowerCase())) {
      console.log(`[Email] Skipping send to ${email} — on unsubscribe list`);
      return { skipped: true, reason: "unsubscribed" };
    }
  } catch (checkErr) {
    // If the check fails, proceed with sending — don't block on this
    console.error("[Email] Unsubscribe check failed:", checkErr.message);
  }

  const resend = getResend();
  const firstName = name.split(" ")[0] || name;

  // ── Custom template: valuation-questionnaire ──────────────────────────────
...
<body style="margin: 0; padding: 0; background-color: #0A0E14; font-family: Arial, Helvetica, sans-serif;">
  <div style="display: none; max-height: 0; overflow: hidden; font-size: 1px; line-height: 1px; color: #0A0E14;">
    We received your answers. Here's how to review your numbers.
    &#847; &#847; &#847; &#847; &#847; &#847; &#847; &#847; &#847; &#847; &#847; &#847; &#847; &#847; &#847;
  </div>
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #0A0E14; padding: 40px 20px;">
    <tr>
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width: 600px; width: 100%;">
          ...
          <tr>
            <td style="background-color: #111720; border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 40px 32px;">
              ...
              <p style="font-size: 14px; color: #8B95A5; margin: 0 0 20px 0; line-height: 1.65;">
                Next step is simple: pick a 30-minute video call time so I can pull your Profit Gap &amp; Valuation Gap using live deal comps specific to your industry and walk you through what it means.
              </p>
              ...
              <table cellpadding="0" cellspacing="0" style="margin: 0 0 28px 0;">
                <tr>
                  <td style="background: linear-gradient(135deg, rgba(200,162,78,0.2), rgba(200,162,78,0.08)); border: 1.5px solid rgba(200,162,78,0.5); border-radius: 12px; padding: 14px 32px;">
                    <a href="https://app.iclosed.io/e/kriczkyvirtus/profit-valuation-snapshot" target="_blank" style="color: #C8A24E; font-size: 12px; font-weight: 700; text-decoration: none; letter-spacing: 0.02em;">
...
async function sendCohortConfirmationEmail({ name, email }) {
  const resend = getResend();
  const firstName = name.trim().split(/\s+/)[0] || name;
  const text = COHORT_CONFIRMATION_TEXT({ firstName });
  const html = buildBrandedEmailHtml({
    firstName,
    email,
    previewText: "Your cohort application came through.",
    heading: "Got your application",
    descriptionHtml: `
              <p style="font-size: 15px; color: #8B95A5; margin: 0 0 20px 0; line-height: 1.6;">
                Your cohort application came through — thanks for taking the time on it.
              </p>
              <p style="font-size: 15px; color: #8B95A5; margin: 0 0 32px 0; line-height: 1.6;">
                I read every one of these personally. If there's a fit, I'll reach out when the next cohort opens with details on timing and seats. If there isn't, I'll tell you that too, and point you somewhere more useful.
              </p>`,
    signoffIntro: "Rooting for you,",
    includeCta: false,
  ...
  console.log(`[Email] Sent cohort application confirmation to ${name} <${email}> — id: ${result.data?.id || "unknown"}`);
  return result;
}

module.exports = { sendResultsEmail, sendCohortConfirmationEmail };
