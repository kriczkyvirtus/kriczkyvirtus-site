...
async function syncContact({
  ...
  if (isReinvestHarvest) {
    if (partial) {
      ...
      try {
        await acFetch("contactTags", "POST", {
          contactTag: { contact: contactId, tag: sourceTagId },
        });
        console.log(`[AC] Tagged partial Reinvest or Harvest contact ${email}: tag id ${sourceTagId}`);
      } catch (tagErr) {
        console.error(`[AC] Failed to apply partial source tag id ${sourceTagId} to ${email}:`, tagErr.message);
      }
