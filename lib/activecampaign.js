...
async function syncContact({
  ...
  if (isReinvestHarvest) {
    if (partial) {
      const tagNames = [
        utmSource
          ? (SOURCE_MAP[utmSource.toLowerCase()] || `Source: ${formatUtmValue(utmSource.toLowerCase())}`)
          : "Source: Website",
      ];
      ...
      for (const tagName of tagNames) {
        try {
          ...
          console.log(`[AC] Tagged partial Reinvest or Harvest contact ${email}: "${tagName}"`);
        } catch (tagErr) {
          console.error(`[AC] Failed to apply partial tag "${tagName}" to ${email}:`, tagErr.message);
        }
      }
