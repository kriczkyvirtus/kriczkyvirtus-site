...
async function syncContact({
  ...
  if (tool === "cohort-waitlist") {
    ...
    const tagIds = [46, 12, 42, ...(tierTagIds[revenueRange] || [])];
    ...
    for (const [fieldId, value] of Object.entries(fieldValues)) {
      try {
        await acFetch("contactFieldValues", "POST", {
          fieldValue: {
            contact: contactId,
            field: fieldId,
            value: value || "",
        ...
        console.log(`[AC] Updated field ${fieldId} for ${email}`);
      } catch (fieldErr) {
        console.error(`[AC] Failed to update field ${fieldId} for ${email}:`, fieldErr.message);
      }
    }
