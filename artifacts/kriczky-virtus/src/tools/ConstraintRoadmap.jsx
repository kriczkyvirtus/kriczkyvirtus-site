...
const QUESTIONS = [
  {
    ...
    // Revenue buckets MUST match V5's six values exactly:
    //   under_500k | 500k_750k | 750k_1m | 1m_3m | 3m_10m | over_10m
    // The $750K split is the qualified/unqualified hero CTA segmentation line.
    options: [
...
export default function ConstraintRoadmap() {
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      ...
      console.log("[Virtus] Lead data:", JSON.stringify({...payload, pdfBase64: pdfBase64 ? "[PDF]" : null}, null, 2));
      const subject = encodeURIComponent("New Lead: Constraint Roadmap — " + name.trim());
      const body = encodeURIComponent("Name: " + name.trim() + "\nEmail: " + email.trim() + "\nScore: " + resultData.score + "%\nConstraint: " + (constraint?.name || "N/A") + "\nBand: " + scoreLabel);
      window.open("mailto:ekriczky@kriczkyvirtus.com?subject=" + subject + "&body=" + body, "_blank");
      setEmailSubmitted(true);
    } finally {
      setSubmitting(false);
    }
