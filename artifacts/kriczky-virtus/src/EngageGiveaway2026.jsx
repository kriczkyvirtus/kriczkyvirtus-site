...
// ═══════════════════════════════════════════════════════════════════
// ENGAGE GIVEAWAY 2026 — Booking Page
// URL: /engage-giveaway-2026
//
// Purpose: Landing page for Chester County business owners from the
// Engage Personal Training giveaway. Offers a free Business Valuation
// Estimate and Profit Gap analysis.
// ═══════════════════════════════════════════════════════════════════
...
export default function EngageGiveaway2026() {
  const { mob } = useBp();
  ...
  const SESSION_ITEMS = [
    {
      ...
      title: "Your Profit Gap — quantified",
      desc: "See the dollar amount your business is leaving on the table each year relative to what it could be earning. Not a guess — a number you can act on.",
    },
    {
      icon: (
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={C.cyan} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <path d="M3 3v18h18" /><path d="M7 14l4-4 4 4 5-7" /><circle cx="20" cy="7" r="1.3" fill={C.cyan} stroke="none" />
        </svg>
      ),
      title: "Your Value Gap — mapped",
      desc: "The difference between what your business is worth today and what an above-average peer in your industry is worth — and why that gap exists.",
    },
    {
      icon: (
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={C.cyan} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <circle cx="12" cy="12" r="10" /><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76" fill={C.cyan} fillOpacity="0.15" />
        </svg>
      ),
      title: "Your #1 constraint identified",
      desc: "Whether you've taken our diagnostic tools or not, we'll pinpoint the single biggest thing suppressing your valuation and cash flow right now.",
    },
    {
      icon: (
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={C.cyan} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" fill={C.cyan} fillOpacity="0.15" />
        </svg>
      ),
      title: "Clarity on what to do next",
      desc: "You leave with a clear picture of where the opportunities are in your business. No commitment required — just actionable insight you can use immediately.",
    },
  ];
      ...
      <div style={{ position: "relative", zIndex: 10, maxWidth: 900, margin: "0 auto", padding: mob ? "25px 20px 60px" : "25px 40px 80px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: mob ? 40 : 56 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 16px", borderRadius: 100, background: "rgba(200,162,78,0.08)", border: "1px solid rgba(200,162,78,0.20)", marginBottom: 20 }}>
            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: mob ? 10 : 12, fontWeight: 600, color: C.gold, letterSpacing: 0.3 }}>Chester County Business Owners from the Engage Personal Training Giveaway</span>
          </div>
          ...
          <p style={{ fontSize: mob ? 14 : 16, lineHeight: 1.65, color: C.text2, maxWidth: 740, margin: "0 auto", fontWeight: 400 }}>
            As a thank you for your generosity towards the community we wanted to offer you a bonus to help you try to grow your business. For a limited time, business owners earning $750K+ in annual revenue can lock in a complimentary Business Valuation Estimate and Profit Gap analysis — for free.
          </p>
        </div>
        ...
        <div style={{ marginBottom: mob ? 40 : 56 }}>
          <div style={{ minHeight: 500, padding: mob ? "20px 16px" : "28px 32px", background: "linear-gradient(145deg, rgba(255,255,255,0.04), rgba(255,255,255,0.015) 50%, rgba(255,255,255,0.025))", border: `1px solid ${C.border2}`, borderTop: "1px solid rgba(255,255,255,0.10)", borderRadius: 18, boxShadow: `0 4px 12px rgba(0,0,0,0.3), 0 16px 48px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.06), 0 0 60px ${C.cyan}0a` }}>
            <iframe
              src="https://app.iclosed.io/e/kriczkyvirtus/engage-giveaway-2026"
              title="Engage Giveaway - Free Valuation Estimate"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: mob ? 40 : 56 }}>
          ...
          {SESSION_ITEMS.map((item, i) => (
            <div key={i} style={{ display: "flex", gap: 16, alignItems: "flex-start", padding: mob ? "16px 18px" : "18px 24px", background: "linear-gradient(145deg, rgba(255,255,255,0.05), rgba(255,255,255,0.02) 50%, rgba(255,255,255,0.03))", backdropFilter: "blur(16px)", border: `1px solid ${C.border2}`, borderTop: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, boxShadow: `0 2px 4px rgba(0,0,0,0.2), 0 8px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.06), 0 0 40px ${C.cyan}08` }}>
              <div style={{ width: 44, height: 44, borderRadius: "50%", flexShrink: 0, background: `${C.cyan}08`, border: `1.5px solid ${C.cyan}30`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 0 12px ${C.cyan}15` }}>
                {item.icon}
              </div>
              <div style={{ flex: 1 }}>
