...
// NAV
// ═══════════════════════════════════════════════════════════════════
const Nav = ({ onPrimary, onWaitlist }) => {
  const { mob } = useBp();
          ...
          <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
            {links.map(l => (
              <a key={l.label} href={l.href}
                style={{ color: l.label === "Community" ? C.gold : C.text2, fontFamily: "'DM Sans',sans-serif", fontSize: 13, fontWeight: l.label === "Community" ? 600 : 500, textDecoration: "none", cursor: "pointer" }}
                onMouseEnter={e => { if (l.label !== "Community") e.target.style.color = C.text1; }}
                onMouseLeave={e => { if (l.label !== "Community") e.target.style.color = C.text2; }}>
                {l.label}
              </a>
            ))}
          </div>
      ...
      <div style={{
        ...
        {links.map(l => (
          <a key={l.label} href={l.href} onClick={() => setMenuOpen(false)}
            style={{ display: "block", padding: "12px 0", color: l.label === "Community" ? C.cyan : C.text2, fontFamily: "'DM Sans',sans-serif", fontSize: 16, fontWeight: l.label === "Community" ? 600 : 500, textDecoration: "none", borderBottom: `1px solid ${C.border1}` }}>
            {l.label}
          </a>
        ))}
...
// ═══════════════════════════════════════════════════════════════════
const PricingSection = ({ onPrimary, onWaitlist }) => {
  const { mob, tab } = useBp();
  const tiers = [
    {
    ...
    },
    {
      ...
      core: [
        ...
        ["Community access", "weekly Q&A to resolve short-term roadblocks that prevent building momentum"],
      ],
      bonuses: [
  ...
  return (
    <Section id="pricing" gradient={`radial-gradient(ellipse 80% 60% at 50% 80%,rgba(200,162,78,0.07) 0%,transparent 55%),linear-gradient(180deg,#080c12 0%,#0a0e15 50%,#090d14 100%)`} style={{ padding: mob ? "60px 0 80px" : "100px 0" }}>
      ...
      <Box>
        ...
        {/* Community teaser */}
        <Reveal delay={500}>
          <div style={{ marginTop: mob ? 14 : 18, padding: mob ? "16px 18px" : "18px 28px", borderRadius: 14, background: `${C.gold}05`, border: `1px solid ${C.gold}18`, display: "flex", flexDirection: mob ? "column" : "row", alignItems: mob ? "flex-start" : "center", gap: 14 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1 }}>
              <IconUsers size={20} color={C.gold}/>
              <div>
                <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: C.text1, fontWeight: 700 }}>Not ready for 1-on-1? A founding community cohort is forming.</div>
                <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, color: C.text3, marginTop: 2 }}>Peer group, shared methodology, and first access to everything we build next.</div>
              </div>
            </div>
            <GhostBtn onClick={onWaitlist}>Join Waitlist</GhostBtn>
          </div>
        </Reveal>
      </Box>
    </Section>
...
const faqs = [
  ...
  { q: "What if I'm not ready for 1-on-1 work?", a: ["Two options: First, use the free diagnostic tools on the Resources Hub — score your business, explore your Profit and Value Gaps, and go deep on specific value drivers. No account needed. Second, ", "join the Build With Valor community — a peer-group format with the same constraint-first methodology, working sessions, and a cohort of owners at your revenue tier.", " Founding member waitlist is open now."] },
...
// ═══════════════════════════════════════════════════════════════════
const FinalCTA = ({ onPrimary, onWaitlist }) => {
  const { mob } = useBp();
  return (
        ...
        <Reveal>
          <Glass intensity="strong" glow={C.gold} style={{ padding: mob ? 28 : 48, textAlign: "center", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              ...
              <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: mob ? 14 : 16, color: C.text2, margin: "0 auto 10px", lineHeight: 1.6, maxWidth: 520 }}>
                The Constraint Roadmap is free. Everything downstream is entered at your pace — never ours.
              </p>
              <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: mob ? 12 : 13, color: C.text3, margin: "0 auto 28px", maxWidth: 520 }}>
                Not ready for 1:1 help? Join the founding community — a private group of owners building together.
              </p>
              <div style={{ display: "flex", gap: 12, justifyContent: "center", alignItems: "center", flexWrap: "wrap" }}>
                <GoldBtn large style={{ width: mob ? "100%" : undefined }} onClick={onPrimary}>Get Your Free Constraint Roadmap</GoldBtn>
                <GhostBtn color={C.cyan} style={{ width: mob ? "100%" : undefined }} onClick={onWaitlist}>Join Community Waitlist</GhostBtn>
              </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "1.5fr 1fr 1fr 1.2fr", gap: mob ? 28 : 40, marginBottom: mob ? 28 : 40 }}>
          ...
          <div>
            ...
            {[
              ...
              { label: "Build With Valor Community", href: "https://buildwithvalor.com" },
            ].map(l => (
              <a key={l.label} href={l.href} style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: C.text3, textDecoration: "none", display: "block", marginBottom: 10, transition: "color 0.2s" }}
                onMouseEnter={e => e.currentTarget.style.color = C.gold}
                onMouseLeave={e => e.currentTarget.style.color = C.text3}
...
export default function KriczkyVirtusHomepage() {
  ...
  return (
    <div id="top" style={{ background: C.bgDeep, minHeight: "100vh", overflow: "hidden" }}>
      ...
      `}</style>
      <Nav onPrimary={scrollToHero} onWaitlist={scrollToCommunity}/>
      ...
      <GoDeeper/>
      <PricingSection onPrimary={scrollToHero} onWaitlist={scrollToCommunity}/>
      {/* CaseStudiesSection hidden until video content is ready */}
      {/* <CaseStudiesSection onPrimary={scrollToHero}/> */}
      <FAQSection/>
      <FinalCTA onPrimary={scrollToHero} onWaitlist={scrollToCommunity}/>
      <ClosingStatement/>
      <Footer/>
    </div>
  );
