...
## Fix 5: Update preview text

In the `TOOL_DETAILS` object, find the `"valuation-questionnaire"` entry and update the `previewText`:

```javascript
previewText: "We received your answers. Here's how to review your numbers.",
```

Also find the hidden preview text div in the custom HTML template:
```html
