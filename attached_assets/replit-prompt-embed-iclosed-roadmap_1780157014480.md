# Replit Agent Prompt — Embed iClosed Calendar on /roadmap-session

## What You Need To Do

In `RoadmapSession.jsx`, make these two changes:
...
Find the div with `id="iclosed-embed"` and all its children (the placeholder with calendar icon, "Calendar loading..." text, and email fallback). Replace that entire block with:
...
- Only modify `RoadmapSession.jsx`.
