# Replit Agent Prompt — Build Unsubscribe Page + API

## Overview
Build an unsubscribe flow: a landing page where users confirm they want to unsubscribe, a serverless function that processes it, and tracking in both Resend's suppression list and Google Sheets.

The email unsubscribe links point to: `https://www.kriczkyvirtus.com/unsubscribe?email=their@email.com`

## Step 1: Create the Unsubscribe page component

Create `Unsubscribe.jsx` in the same directory as the other page components:
...
```jsx
import Unsubscribe from "./Unsubscribe"; // adjust path as needed
...
Create `api/unsubscribe.js` at the repo root:
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      // Add to suppression list so Resend blocks future sends
      ...
      // If Resend audience isn't set up yet, just log it
...
## Step 4: Check unsubscribe list before sending emails

In `lib/email.js`, update the `sendResultsEmail` function to check the Google Sheets "Unsubscribed" tab before sending. Add this check at the beginning of the function, before the `resend.emails.send()` call:

```javascript
// Check if email is on the unsubscribe list
try {
  const { google } = require("googleapis");
  const credentials = JSON.parse(process.env.GOOGLE_SHEETS_CREDENTIALS);
  const auth = new google.auth.GoogleAuth({
...
## Step 5: Create the "Unsubscribed" tab in Google Sheets

**Manual step for Edward:** In your Google Sheet (`Kriczky Virtus Website — Lead Capture`), add a new tab called `Unsubscribed` with these column headers in Row 1:
...
- The unsubscribe page must match the Virtus design system (dark background, gold accents, Cormorant headings)
- The serverless function logs to BOTH Resend and Google Sheets — if either fails, the other still runs
- The email check before sending reads from the Sheets "Unsubscribed" tab — if the check fails (network error, etc.), the email still sends rather than silently failing
- Do NOT change any existing email templates, lead capture logic, or other pages
- The Resend audience/suppression may fail if `RESEND_AUDIENCE_ID` isn't set — that's OK for now, the Sheets logging is the primary record
