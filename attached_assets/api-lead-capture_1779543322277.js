...
// Sends to: Resend (email with PDF), ActiveCampaign (CRM), Google Sheets (logging)
...
export const config = {
  api: {
    bodyParser: {
      sizeLimit: '4mb', // PDFs can be 1-2MB
    },
  },
};
...
async function updateAggregatedTab(sheetsClient, spreadsheetId, email, toolName) {
  try {
    // Read existing aggregated data to check if this email already exists
    ...
    if (emailColIdx === -1) {
      // Headers don't exist yet — this is the first entry
      return false;
    }
    ...
    if (existingRowIdx > -1) {
      // Update existing row — append this tool to their "Tools Completed" list
...
async function syncToActiveCampaign(acUrl, acKey, { name, email, tool, toolName, summary }) {
  try {
    ...
    // Step 2: Find or create tag for this tool
    ...
    await fetch(`${acUrl}/api/3/notes`, {
      ...
      headers: { 'Api-Token': acKey, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        note: { note: noteText, relid: contactId, reltype: 'Subscriber' },
      }),
    });
        ...
        html: `
          <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; color: #333;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
              ...
              <tr>
                <td style="padding: 8px 12px; font-weight: 600; color: #666; border-bottom: 1px solid #eee;">Tool</td>
                <td style="padding: 8px 12px; border-bottom: 1px solid #eee;">${toolName} (${tool})</td>
              </tr>
              ${summary.pct ? `
              <tr>
                <td style="padding: 8px 12px; font-weight: 600; color: #666; border-bottom: 1px solid #eee;">Score</td>
                <td style="padding: 8px 12px; border-bottom: 1px solid #eee;"><strong>${summary.pct}%</strong></td>
              </tr>
              ` : ''}
              ${summary.band ? `
              <tr>
                <td style="padding: 8px 12px; font-weight: 600; color: #666; border-bottom: 1px solid #eee;">Band</td>
                <td style="padding: 8px 12px; border-bottom: 1px solid #eee;">${summary.band}</td>
              </tr>
              ` : ''}
            </table>
...
export default async function handler(req, res) {
  ...
  // Track results
  const results = { resend: false, activecampaign: false, sheets: false };
  ...
  if (SHEETS_ID && SERVICE_EMAIL && PRIVATE_KEY) {
    try {
      ...
      // Update the "Tools Completed" field for this email in Aggregated
      if (aggResult) {
        await updateAggregatedTab(sheetsClient, SHEETS_ID, email, toolName);
      }
  ...
  // Return success even if some services failed — the frontend should unlock results
  // regardless, and the mailto fallback handles total failure
